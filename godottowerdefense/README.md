...
| Thing | Where | Value |
|---|---|---|
| Difficulty rulesets | `balance.gd` `RULESETS` | Per-profile hp / count / speed multipliers plus start gold & lives, cycled by the menu button. **easy** 0.70 / 0.85 / 0.95, 150g / 25 lives · **normal** 1.0 / 1.0 / 1.0, 120g / 20 lives (the tuned baseline) · **hard** 1.15 / 1.08 / 1.06, 110g / 18 lives. `--play-sim` (the floor player, no abilities used) loses around wave 35 on normal and wave 27 on hard; `--fill-board` (a maxed board) still clears wave 50 |
| Starting gold / lives | `balance.gd` `RULESETS` | Normal 120g / 20 lives (the map gives 30g, but its towers are researched rather than bought). Per-ruleset, read at run start |
...
| Enemy speed / durability | `balance.gd` `CREEP_SPEED_PERCENT` / `CREEP_HP_PERCENT` | every board uses ×0.82 movement speed for clearer motion and ×1.36 HP. The HP figure is the main difficulty dial and was reshaped — a gentle opening (waves 1-25 leak nothing for the floor player) with a steeper finish — so a run punishes a board that does not use its abilities rather than one that misplays the first minute |
...
| Wave scaling (`n` = wave) | `balance.gd` | HP `225 × hp_growth()^(n-1) × 1.36`, where `hp_growth()` is derived so wave `STANDARD_WAVES` lands on `FINAL_HP_FACTOR` (100× wave 1). Both ends have moved several times; the current pair (55→100 finish, 1.20→1.36 flat) was set by the difficulty pass — a lower flat floor for a soft opening, a higher endpoint for a hard finish, read off `--play-sim` and gated by `--fill-board` still clearing wave 50. Speed `(80 + (n-1)·speed_slope()) × 0.82`, derived so the last wave reaches `FINAL_SPEED_RAW` 260 raw = 213 px/s = a 15s crossing of the 3199px road. Count ramps `12 + n` to the map's flat 28 cap (ruleset multiplier applied outside the cap, so hard's +8% is not dead past wave 16). Reward `3 + round(n·0.68)` (`REWARD_SLOPE`, trimmed from a flat `3 + n` so the board a run can afford is bounded by income, not just by open ground). Each × the archetype's multipliers |
