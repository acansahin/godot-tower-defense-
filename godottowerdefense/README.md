...
**painted** board and the fire, water and nature towers, five tiers each (`assets/art/`),
and the **code-drawn** everything else — enemies, the other three elements, projectiles,
every effect and all of the UI — still built from primitive shapes in `_draw()`. Every sound is still synthesized
...
- **`Tower`** is one generic script. `setup_def(id)` loads a `Game.TOWER_DEFS`
  ...
  fire, water and nature, all five tiers) and from the code art if not, which lets the
...
- **The fire, water and nature towers**, five tiers each: `towers/<element>_1..5.png`, cut
  from one generated sheet (`_source_<element>.png`) by `tools/cut_sprites.py` at
  `max_height` 220, which puts the tiers at ~2x their drawn size. Sets are generated
  five-at-a-time on
...
- **Not yet painted**: light, darkness and earth, all fifteen duals, and every enemy. They
