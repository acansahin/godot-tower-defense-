...
wave without leaking away all twenty lives first), `--creep-pose:<archetype>` (the same idea
for ANY row of `Game.WAVE_TYPES`: five of that archetype parked along an empty road with a
plain Normal walking beside each, damaged so the health bars move. It sets the traits through
the SAME `WAVE_TYPES` fields `WaveManager._spawn_one` reads, so what it photographs is the
ROW's behaviour and not the harness's. This is the only way to see the Warden's ground aura
and the ring it puts on whoever stands in it, or the Wisp's chevrons filling up before it
jumps — those archetypes first appear on wave 21, and `--fill-board` kills every creep within
a frame of its spawn), `--boss-pose` (stages both bosses' rules —
...
| Wave (first 24 only) | `Game.WAVES` — past that, waves are generated. **No boss goes in this table**, see below. The table's job is to TEACH: every archetype gets one wave of its own before the generator is allowed to roll it, which is why adding an archetype means adding a row here as well as a `POOL` entry |
...
| Creep archetype | a row in `Game.WAVE_TYPES`, a teaching row in `Game.WAVES`, and a `WaveGenerator.POOL` entry whose `from` is the wave after the teaching one. A row that is only multipliers and existing flags needs **no code**; a new MECHANIC needs a field read in `WaveManager._spawn_one`, a var + a tick in `Enemy`, and a mark in `Enemy._draw_overlay` — see `heal_aura` (Warden) and `blink_distance` (Wisp) for the shape of both. Give the row an `art` key naming a painted set to borrow: `Enemy.art_kind()` uses it only while `<key>_1.png` is missing, so the archetype ships playable and becomes its own creature later with no code change. Check the drawing with `--creep-pose:<key>` |
| Variety in the back half of a run | `WaveGenerator`, three knobs that do different jobs and are worth keeping separate. `POOL`'s `late` weights re-aim the mix across waves 25-45, so a late roll is mostly the archetypes that ask a QUESTION rather than more Normals. `REPEAT_WINDOW` forbids an archetype repeating within 2 waves — independent rolls are supposed to clump, and no weight tuning fixes a clump. `ESCORT_*` is the big one: from wave 26 a wave may carry a SECOND archetype as a third of its count, spread through the spawn order by `WaveManager._build_plan()`, which takes ten kinds of wave to ninety and changes the fight rather than the roster. Escorts are kept off boss waves, and an air escort is forced NEUTRAL (`make_flying()` overwrites the tint, so an element on a flyer is armour the player cannot see) |
...
| Painted creep | `assets/art/enemies/<archetype>.png`, named for its `Game.WAVE_TYPES` key (`normal.png`, `tank.png`, …). **No code change** — `sprites.gd` `enemy()` finds it and `enemy.gd` prefers it over the blob, and prefers it over the row's borrowed `art` set too, so painting `warden`/`wisp` is a pure file drop. Art faces SCREEN-LEFT and is mirrored by `_facing`. A boss is an archetype wearing a crown, with ONE exception: the four element avatars take `boss_<element>_1..N.png` if those exist (`Enemy.art_kind()` picks them off `avatar_element` and falls back to the crowned archetype otherwise), so the four can be painted one at a time — check them with `--avatar-pose`. Numbered files (`normal_1.png`…`normal_6.png`) are an animation cycle of ANY length — `Sprites.pose_count()` counts them and both carriers divide their cycle by the answer, so re-animating a creep is a file copy; one file alone is a still. Only the `"air"` row in `WAVE_TYPES` flies, and its cycle is a WINGBEAT, not a stride. Generate sheets from [docs/creep-art-prompt.md](godottowerdefense/docs/creep-art-prompt.md) — one creature per sheet, one frame per row, TWELVE of them: the cycle is one stride played at the creep's own walking rate, so six frames measured 6.2 fps at wave 2 and the eye counts them. **The pace comes from the STRIDE the art was drawn with** (`Sprites.stride()`, the widest gap between the feet), not from the creep's radius: one cycle carries the creature the two steps it is painted taking, so a sheet drawn with a lunging stride walks slowly and one drawn at the roster's 0.6-0.9x of body height walks at a normal rate. `Enemy.WALK_TEMPO` (1.35) is the one deliberate lie — a third faster than the feet, which buys frame rate for a slip too small to read — and `Enemy.FRAME_BLEND` dissolves each pose into the next so a 5 fps cycle does not read as a slide show |
